...
{
public:
    JsLogicAnalyzer(Nan::Callback *progress, Nan::Callback *complete, Nan::Callback *error,
                    const v8::Local<v8::Object> &options);

    void Execute( const Nan::AsyncProgressWorker::ExecutionProgress& progress) override;

    template <typename Value>
    std::string serialize( const std::string &label, const std::vector<Value>& results);
